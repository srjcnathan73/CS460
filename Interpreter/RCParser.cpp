...
RCParser::RCParser(std::vector<char> *fileBuffer,std::string fileName)
{
...
bool RCParser::checkForReservedWords(const std::string& word)
{
...
bool RCParser::checkForNegativeInteger(const std::string& num)
{
    int int_num = std::stoi(num);
    return int_num < 0;
}

bool RCParser::checkIsOperator(const std::string& type)
{
    return type == "MODULO"||type == "ASTERISK"||type == "PLUS"||type == "MINUS"||type == "DIVIDE"||type == "CARET"||type == "LT"||type == "GT"||type == "BOOLEAN_NOT";
}

void RCParser::printCSTUtility(CSTNode* node)
{
    if(node == nullptr){
        return;
    }
...
void RCParser::printCST()
{
    printCSTUtility(root);
}

void RCParser::breadthFirstPrint() {
    if (!root) return;
...
void RCParser::breadthFirstFilePrint(std::string inputFileName) {
